...
$activity_reports->add_report(
	__( 'User Posts', 'postindexer' ),
	'user-posts',
	__( 'Posts created or updated by a user.', 'postindexer' )
);
	...
	switch( $action ) {
		...
		case 'view':
			...
			if ( ! $user ) {
				...
				// === ZUSAMMENFASSUNG ===
				$total_posts = array_sum( $counts_by_date );
				echo '<div style="background: #f8f9fa; border-left: 4px solid #0073aa; padding: 20px; margin: 20px 0; border-radius: 4px;">';
				echo '<h3 style="margin-top: 0; color: #0073aa;">' . esc_html__('Bericht: User Posts', 'postindexer') . '</h3>';
				echo '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">';
				
				echo '<div>';
				echo '<div style="font-size: 28px; font-weight: bold; color: #0073aa;">' . intval($total_posts) . '</div>';
