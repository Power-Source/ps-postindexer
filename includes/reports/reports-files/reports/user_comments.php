...
$activity_reports->add_report(
	__( 'User Comments', 'postindexer' ),
	'user-comments',
	__( 'Comments created by a user.', 'postindexer' )
);

if (
    (isset($_GET['report']) && $_GET['report'] === 'user-comments') ||
    (isset($_POST['report']) && $_POST['report'] === 'user-comments')
) {
...
function report_user_comments_ouput() {
	...
	switch( $action ) {
		...
		default:
			...
			<form name="report" method="post" action="?page=reports&action=view-report&report=user-comments&report-action=view">
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php esc_html_e( 'Benutzername', 'postindexer' ); ?></th>
						<td>
							...
							<br />
							<?php esc_html_e( 'Mindestens 2 Zeichen eingeben, um Vorschlaege zu laden.', 'postindexer' ); ?>
						</td>
					</tr>
					<tr valign="top">
		...
		case 'view':
			...
			if ( ! $user ) {
				?>
				<p><?php esc_html_e( 'Benutzer nicht gefunden.', 'postindexer' ); ?></p>
                <?php
			} else {
				$user_ID = $user->ID;
				?>
                <p>
                    <ul>
						<li><strong><?php esc_html_e( 'Benutzer', 'postindexer' ); ?></strong>: <?php echo esc_html( $user_login ); ?></li>
				...
				// === ZUSAMMENFASSUNG ===
				$total_comments = array_sum( $counts_by_date );
				echo '<div style="background: #f8f9fa; border-left: 4px solid #0073aa; padding: 20px; margin: 20px 0; border-radius: 4px;">';
				echo '<h3 style="margin-top: 0; color: #0073aa;">' . esc_html__('Bericht: User Kommentare', 'postindexer') . '</h3>';
				echo '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">';
				
				echo '<div>';
				echo '<div style="font-size: 28px; font-weight: bold; color: #0073aa;">' . intval($total_comments) . '</div>';
				echo '<div style="color: #666; font-size: 13px;">' . esc_html__('Kommentare in den letzten ' . intval($period) . ' Tagen', 'postindexer') . '</div>';
				echo '</div>';
				...
				if ( !empty($counts_by_date) ) {
					...
					echo '<div style="color: #666; font-size: 13px;">' . esc_html__('Kommentare am neuesten Datum (' . date_i18n('d. M Y', strtotime($latest_day)) . ')', 'postindexer') . '</div>';
					echo '</div>';
				}
				...
				if ( !empty($array_values) && $highest_value > 0 ) {
					...
					echo '<h4>' . esc_html__('Grafik: Kommentare über Zeit', 'postindexer') . '</h4>';
